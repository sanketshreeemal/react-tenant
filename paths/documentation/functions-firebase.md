...
### 42. `getDelinquentUnitsForDashboard`

*   **Function Parameters:**
    *   `landlordId: string | undefined | null` - The ID of the landlord.
    *   `currentSystemDate: Date` - The current system date, used for arrears calculation.
    *   `propertyGroupId?: string` - Optional ID of a property group to filter results.
*   **Function Output:** `Promise<DelinquencyDashboardData>` - Data for the delinquent units dashboard.
*   **Explanation:** Calculates and retrieves information about delinquent rental units for the dashboard. This function identifies units with unpaid rent from previous months, considering active leases only, a historical data hard stop, and arrears logic.

---

### 43. `groupLeasesByProperty`
